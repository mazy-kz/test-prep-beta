...
export async function POST(req: Request) {
  try {
    ...
    if (!subjectId) return NextResponse.json({ ok:false, message:'Missing subjectId.' }, { status:400 });
    ...
    const XLSXmod = XLSX; // type quiet
    const wb = XLSXmod.read(buf, { type: 'buffer' });
    const sheetName = wb.SheetNames?.[0];
    if (!sheetName) return NextResponse.json({ ok:false, message:'No sheet found.' }, { status:400 });

    const ws = wb.Sheets[sheetName];
    const rows: any[][] = XLSXmod.utils.sheet_to_json(ws, { header: 1 });
    if (!rows?.length) return NextResponse.json({ ok:false, message:'Sheet is empty.' }, { status:400 });
    ...
    const batch: {
      ...
      choiceA: string; choiceB: string; choiceC: string | null; choiceD: string | null;
      correct: Letter; comment: string | null;
    }[] = [];
    ...
    for (let i = start; i < rows.length; i++) {
      ...
      const correctText = (r[1] ?? '').toString().trim();
      const optA = (r[2] ?? '').toString().trim();
      const optB = (r[3] ?? '').toString().trim();
      const optC = (r[4] ?? '').toString().trim();
      const comment = (r[5] ?? '').toString().trim();
      ...
      const correctIdx = options.findIndex(o => o.trim().toLowerCase() === correctText.toLowerCase());
      if (correctIdx < 0) { problems.push(`Row ${rowNum}: correct text (B) not found among options C–E.`); continue; }
    ...
    if (!batch.length) {
      return NextResponse.json({ ok:true, created:0, problems: problems.length ? problems : ['No valid rows found.'] });
    }

    const CHUNK = 500;
    for (let i = 0; i < batch.length; i += CHUNK) {
      await prisma.question.createMany({ data: batch.slice(i, i + CHUNK) });
    }
